...
- Выявлять моменты неопределенности модели (низкая уверенность)
...
2. Запустите оба варианта и сравните:
   - **Средняя уверенность** — якорение должно повышать уверенность модели
   - **Top-1 вероятность** — анализируйте, как меняются вероятности ключевых токенов
   - **Выбор токена** — отслеживайте первые 5-10 токенов: якорение должно сместить выбор в сторону более специфичной лексики

**Ожидаемый результат:** При эффективном якорении вы увидите:
- Повышение средней уверенности на 5-15%
...
2. Анализируйте метрики:
   - **Min уверенность** — структурированный формат должен повышать минимальную уверенность
   - **Распределение уверенности** — смотрите на процент большой уверенность (≥90%) токенов
...
2. Отслеживайте токены:
   ...
   - **Анализируйте уверенность на числовых токенах** — CoT должен повышать уверенность на вычислениях
   - **Проверяйте последовательность** — модель должна генерировать токены в логическом порядке

**Ожидаемый результат:**
- Появление промежуточных рассуждений с высокой уверенностью (>85%)
...
**Ожидаемый результат:**
- 3-shot промты показывают на 20-30% выше среднюю уверенность
- Снижение вариативности в выборе токенов
- Более строгое следование формату примеров


### 5. Контекстное окно и Attention Decay
...
3. Отслеживайте:
   - **Confidence on answer tokens** — информация из начала/конца имеет выше уверенность
   - **Structural token appearance** — модель может "потерять" структуру в середине длинного контекста

**Lost in the middle problem:**
- Если ключевая информация в середине длинного контекста, и вы видите низкую уверенность на токенах ответа — это классический "lost in the middle"
...
### Scenario 2: A/B тестирование промтов для production
